...
from api.models.poll_dtos import PollDto, PollItemDto
from flask import make_response, request, Blueprint, Response
from flask_restful import Api, Resource
from prisma.errors import UniqueViolationError
from werkzeug.exceptions import BadRequest

poll = Blueprint("poll", __name__, url_prefix="/poll")
poll_api = Api(poll)
...
class PollItems(Resource):
    def get(self, poll_id):
        try:
            poll = Poll.prisma().find_first(where={"id": poll_id}, include={"items": True})
        except UniqueViolationError:
            raise BadRequest
        
        response_data = []
        for poll_item in poll.items:
            response_data.append({"description":poll_item.description, "votes":poll_item.votes})
...
poll_api.add_resource(PollCreate, "/create")